...
int fitness(const vector<vector<int>>& G, double p, const vector<int>& solution) {
	...
	int fitnes = G.size() - simulate_IC(G, active, p);
	return fitnes;
}
...
vector<vector<int>> nextGeneration(const vector<vector<int>>& G, vector<vector<int>>& population, double mutationProbability, int tournamentSize, const double& p) {
...
int main(int argc, char **argv) {
	...
	float bestFitness = fitness(G, p, bestSolution);
