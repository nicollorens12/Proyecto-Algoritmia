...
vector<int> random_set(int V, double p) {
    vector<int> aux;
...
bool its_valid(const vector<vector<int> >& G, const vector<int>& sol, double p) {
    int V = G.size();
    vector<int> S(sol);
...
vector<int> local_searchIC(vector<vector<int> > G, double p) {
    ...
    vector<int> rand_set = random_set(V, p);
    vector<bool> active(V, false);

    while (rand_set.empty() || !its_valid(G, rand_set, p)) {
        rand_set = random_set(V, p);
    }
    ...
    while (iteration < max_iterations && quality < V) {
        ...
        if (its_valid(G, neighbor_S, p) && (neighbor_quality > quality || (neighbor_S.size() == S.size() && more_degree(S, neighbor_S, G)))) {
            S = neighbor_S;
